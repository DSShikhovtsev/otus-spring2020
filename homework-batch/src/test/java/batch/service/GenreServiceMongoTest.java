...
import batch.service.mongo.GenreService;
...
class GenreServiceMongoTest {
    ...
    @MockBean
    private GenreMongoRepository repository;
    ...
    void setUp() {
        ...
        Mockito.when(repository.findById("1")).thenReturn(new Genre("1", "genre")).thenReturn(null);
        given(repository.findById("2")).willReturn(new Genre("2", "update"));
        given(repository.findById("3")).willReturn(new Genre("3", "test"));
    ...
    void updateGenre() {
        Genre genre = new Genre("2", "genre1");
